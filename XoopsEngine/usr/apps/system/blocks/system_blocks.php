...
function b_system_online_show()
{
    ...
    $online_handler = XOOPS::getHandler('online');
...
function b_system_main_show()
{
    ...
    $modules = XOOPS::service('registry')->modulelist->read("active", "front");
    //Debug::e($modules);
    //$acl = new Xoops_Acl("front");
    //$clause = new xoops_Zend_Db_Clause("resource IN (?)", array_keys($modules));
    //$modulesAllowed = $acl->getResources($clause);
    //$resources = XOOPS::service("registry")->resource->read("front");
    //$moduleperm_handler = XOOPS::getHandler('groupperm');
    //$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
    //$read_allowed = $moduleperm_handler->getItemIds('module_read', $groups);
    foreach ($modules as $dirname => $module) {
        /*
        if (empty($module['active']) || !in_array($module['mid'], $read_allowed)) {
            continue;
        }
        */
        /*
        if (empty($module['active']) || (!is_null($modulesAllowed) && !in_array($dirname, $modulesAllowed))) {
            continue;
        }
        */
        $block['modules'][$dirname]['name'] = $module['name'];
        $block['modules'][$dirname]['directory'] = $dirname;
        if ($module['type'] == "legacy") {
            $block['modules'][$dirname]['link'] = XOOPS::assembleUrl(array('module' => $dirname), 'legacy');
        } else {
            $block['modules'][$dirname]['link'] = XOOPS::assembleUrl(array('module' => $dirname), 'default');
        }
        /*
...
function b_system_user_show()
{
    ...
    $block['uid'] = XOOPS::registry('user')->id;
        ...
        if ($xoopsDB->getRowsNum($result) > 0) {
            ...
            while  ($userinfo = $xoopsDB->fetchArray($result)) {
                ...
                if (isset($xoopsUser) && is_object($xoopsUser)) {
                    $block['groups'][$i]['users'][] = array('id' => $userinfo['uid'], 'name' => $myts->htmlspecialchars($userinfo['uname']), 'msglink' => "<a href=\"javascript:openWithSelfMain('".XOOPS_URL."/pmlite.php?send2=1&amp;to_userid=".$userinfo['uid']."','pmlite',450,370);\"><img src=\"".XOOPS_URL."/images/icons/pm_small.gif\" border=\"0\" width=\"27\" height=\"17\" alt=\"\" /></a>", 'avatar' => $GLOBALS['xoops']->url('www/uploads').'/'.$userinfo['user_avatar']);
                } else {
                    if ($userinfo['user_viewemail']) {
                        $block['groups'][$i]['users'][] = array('id' => $userinfo['uid'], 'name' => $myts->htmlspecialchars($userinfo['uname']), 'msglink' => '<a href="mailto:'.$userinfo['email'].'"><img src="'.XOOPS_URL.'/images/icons/em_small.gif" border="0" width="16" height="14" alt="" /></a>', 'avatar' => $GLOBALS['xoops']->url('www/uploads').'/'.$userinfo['user_avatar']);
                    } else {
                        $block['groups'][$i]['users'][] = array('id' => $userinfo['uid'], 'name' => $myts->htmlspecialchars($userinfo['uname']), 'msglink' => '&nbsp;', 'avatar' => $GLOBALS['xoops']->url('www/uploads').'/'.$userinfo['user_avatar']);
                    }
                }
                $i++;
            }
...
function b_system_newmembers_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        if ( $options[1] == 1 ) {
            $block['users'][$i]['avatar'] = $newmembers[$i]->getVar('user_avatar') != 'blank.gif' ? $GLOBALS['xoops']->url('www/uploads').'/'.$newmembers[$i]->getVar('user_avatar') : '';
        } else {
...
function b_system_topposters_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ( $options[1] == 1 ) {
            $block['users'][$i]['avatar'] = $topposters[$i]->getVar('user_avatar') != 'blank.gif' ? $GLOBALS['xoops']->url('www/uploads').'/'.$topposters[$i]->getVar('user_avatar') : '';
        } else {
            $block['users'][$i]['avatar'] = '';
        }
...
function b_system_comments_show($options)
{
    ...
    $comment_handler = XOOPS::getHandler('comment');
...
function b_system_notification_show()
{
    ...
    $notification_handler = XOOPS::getHandler('notification');
...
function b_system_themes_show($options)
{
    ...
    $block = array();
    if ($options[0] == 1) {
        $block['theme_select'] = "<img vspace=\"2\" id=\"xoops_theme_img\" src=\"".$GLOBALS['xoops']->url('themes')."/".$xoopsConfig['theme_set']."/shot.gif\" alt=\"screenshot\" width=\"".intval($options[1])."\" /><br /><select id=\"xoops_theme_select\" name=\"xoops_theme_select\" onchange=\"showImgSelected('xoops_theme_img', 'xoops_theme_select', 'themes', '/shot.gif', '".XOOPS_URL."');\">".$theme_options."</select><input type=\"submit\" value=\""._GO."\" />";
    } else {
