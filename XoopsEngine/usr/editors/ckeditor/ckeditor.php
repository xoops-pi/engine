...
include __DIR__ . '/editor/ckeditor.php';
...
class ckeditor extends \CKEditor
{
    ...
    public function editor($name, $value = "", $config = array(), $events = array())
    {
        ...
        $replaceTag = empty($this->textareaAttributes['id']) ? $name : $this->textareaAttributes['id'];
        if (!empty($_config))
            $js .= "CKEDITOR.replace('".$replaceTag."', ".$this->jsEncode($_config).");";
        else
            $js .= "CKEDITOR.replace('".$replaceTag."');";
