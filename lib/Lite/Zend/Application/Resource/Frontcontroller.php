...
class Lite_Zend_Application_Resource_Frontcontroller extends Zend_Application_Resource_Frontcontroller
{
    ...
    public function init()
    {
        ...
        foreach ($modules as $dirname => $data) {
            ...
            if (!isset($options['modulelist'][$dirname])) {
                $options['modulelist'][$dirname] = (("app" == $data["type"]) ? "app" : "module") . "/" . $data["directory"];
            }
        }
            ...
            switch (strtolower($key)) {
                ...
                case 'plugins':
                    foreach ((array) $value as $key => $plugin) {
                        ...
                        if (class_exists("Lite_" . $pluginClass)) {
                            $pluginClass = "Lite_" . $pluginClass;
        ...
        Xoops_Zend_Controller_Action_HelperBroker::setPluginLoader(
            new Xoops_Zend_Loader_PluginLoader(array(
                'Zend_Controller_Action_Helper'         => XOOPS::path('lib') . '/Zend/Controller/Action/Helper/',
                'Lite_Zend_Controller_Action_Helper' => XOOPS::path('lib') . '/Lite/Zend/Controller/Action/Helper/',
            ))
        );
