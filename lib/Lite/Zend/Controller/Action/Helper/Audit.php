...
class Lite_Zend_Controller_Action_Helper_Audit extends Zend_Controller_Action_Helper_Abstract
{
    /**
    ...
    public function getAuditPlugin()
    {
        if (null === $this->auditPlugin) {
            ...
            if (!$front->hasPlugin('Lite_Zend_Controller_Plugin_Audit')) {
                $front->registerPlugin(new Lite_Zend_Controller_Plugin_Audit());
            }
            $this->auditPlugin = $front->getPlugin('Lite_Zend_Controller_Plugin_Audit');
        }
